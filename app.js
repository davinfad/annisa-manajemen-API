...
app.delete('/transaksi/:id_transaksi', (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      ...
      // Retrieve transaction details including id_cabang, status, and transaction date
      const getTransaksiSql = 'SELECT id_cabang, status, created_at FROM transaksi WHERE id_transaksi = ?';
      connection.query(getTransaksiSql, [idTransaksi], (err, transaksiResults) => {
        if (err) {
        ...
        connection.query(getWorkingHoursSql, [id_cabang], (err, cabangResults) => {
          if (err) {
            return connection.rollback(() => {
          ...
          const { jam_buka, jam_tutup } = cabangResults[0];
          const openingTime = moment(jam_buka, 'HH:mm:ss');
          const closingTime = moment(jam_tutup, 'HH:mm:ss');
          const transactionTime = moment(created_at, 'YYYY-MM-DD HH:mm:ss');

          const isOutsideWorkingHours = transactionTime.isBefore(openingTime) || transactionTime.isAfter(closingTime);

          // Retrieve items associated with the transaction
          const getItemsSql = 'SELECT * FROM item_transaksi WHERE id_transaksi = ?';
          connection.query(getItemsSql, [idTransaksi], (err, items) => {
            if (err) {
              return connection.rollback(() => {
                res.status(500).send('Error retrieving transaction items!');
                connection.release();
            ...
            const updateKomisiPromises = items.map(item => {
              return new Promise((resolve, reject) => {
                const getKomisiSql = 'SELECT persen_komisi, persen_komisi_luarjam FROM layanan WHERE id_layanan = ?';
            ...
            Promise.all(updateKomisiPromises)
              .then(deleteTransactionItems)
              .catch(error => {
                connection.rollback(() => {
                  res.status(500).send('Error updating commissions!');
                  connection.release();
                  console.error(error);
                });
              });
      ...
      function deleteTransactionItems() {
